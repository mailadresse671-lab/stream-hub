...
// Modul-weiter In-Memory-Cache, gleiches Muster wie api/twitch-followers.js -
// vermeidet unnoetige Token-Refreshs, falls mehrere Quellen (oldschool-
// chatting.html o.ae.) kurz hintereinander laden.
...
// Liefert die aktuelle Gesamt-Abonnentenzahl ueber Twitch Helix
// (GET /subscriptions, "total"-Feld direkt in der Antwort, kein Paging
// noetig) - braucht denselben channel:read:subscriptions-Scope, der fuer
// channel.subscription.message-EventSub ohnehin schon hinterlegt ist (siehe
// CLAUDE.md), kein neuer Twitch-App-Scope noetig.
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) {
      const text = await response.text();
      throw new Error(`Sub-Abfrage fehlgeschlagen (${response.status}): ${text}`);
    }
