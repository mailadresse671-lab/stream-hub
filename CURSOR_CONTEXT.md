...
0. **"90s Old School Edition"-Szenen noch nicht live in Streamlabs Mobile validiert** (`oldschool-starting/-chatting/-gameplay/-brb/-outro.html`, neu) — nur per Playwright/Screenshot geprüft. Insbesondere zu prüfen: F1/F4/F5s atmosphärischer CSS-Hintergrund vs. optionale eigene Medienquelle im selben Szenen-Slot, F2/F3s transparente Facecam-/RL-Rahmen tatsächlich lückenlos über der jeweiligen echten Quelle, F3s Safe-Zone-Koordinaten gegen das echte Rocket-League-HUD (nur grob geschätzt), sowie ob `api/twitch-subs.js` nach Deploy tatsächlich `success:true` mit dem korrekten `total`-Wert liefert.
