...
1. **`api/stats.js` / Rocket-League-Stats-Offline-Status — mehrstufig behoben, letzter Verifikationsschritt offen**: Header-Fix gegen die Tracker.gg-Bot-Sperre wirkungslos → Umstieg auf die offizielle Tracker-Network-API (`TRACKER_API_KEY`) → erster URL-Versuch (`/v2/...`) lieferte `trackerStatus: 401` ("Invalid authentication credentials") → korrigiert auf den in der offiziellen Doku (`tracker.gg/developers/docs/data-ingest`) dokumentierten Pfad `https://public-api.tracker.gg/api/v1/rocket-league/standard/profile/{platform}/{playerId}`. Die Segment-Parsing-Logik (`overview`/`playlist`, `findRank()`) wurde unverändert übernommen — **aus der Sandbox nicht gegen den echten Key testbar**. Falls `/api/stats` nach diesem Deploy weiterhin `success:false` mit `foundSegmentTypes` statt echter Werte liefert, muss die Segment-Struktur der offiziellen Antwort manuell geprüft und `extractRank()`/die `overview`-Feldnamen ggf. angepasst werden.
