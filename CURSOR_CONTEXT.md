...
Übergabe-Snapshot für die Weiterarbeit an **stream-hub** — trotz des Dateinamens **werkzeugunabhängig** (Cursor, VS Studio/Copilot, Aider, oder jedes andere Tool/jede andere KI kann direkt hiermit weiterarbeiten, ohne Claude Code). Dieses Dokument ist eine strukturierte Zusammenfassung — die vollständige chronologische Historie (jede Fehlerkorrektur-Runde, jede Nutzer-Rückmeldung) steht in [`CLAUDE.md`](./CLAUDE.md). Bei Widersprüchen zwischen den beiden Dateien gilt: **dieses Dokument beschreibt den aktuellen Ist-Zustand**, `CLAUDE.md` erklärt, wie es dazu kam. Kompakte Szenen-Tabelle zum schnellen Scannen: [`SZENEN_UEBERSICHT.md`](./SZENEN_UEBERSICHT.md).
...
4. **"90s Old School Edition"** — `oldschool-starting.html`, `oldschool-chatting.html`, `oldschool-gameplay.html`, `oldschool-brb.html`, `oldschool-outro.html`. Schwarz/Weiß-Basis + Blutrot (`#C8102E`/`#8C0A1F`) als einzigem Akzent, `Rubik Spray Paint` (Graffiti-Spray-Schrift) + `Permanent Marker` (Sticker-Texte) + `Barlow Condensed` (Fließtext), 1:1 aus dem Claude-Design-Export "DaNgsxr1000 Stream Overlay Suite" (`<x-dc>`-Wrapper-Format) übersetzt. Eigenständige Ergänzung, komplett getrennt von den beiden anderen Systemen — siehe Abschnitt 5.
...
| :--- | :--- | :--- |
| `api/stats.js` | Rocket-League-Stats von der offiziellen Tracker-Network-API (`public-api.tracker.gg/api/v1/rocket-league/standard/profile/...`, Header `TRN-Api-Key`) für Spieler `DaNgsxR` (Plattform `epic`) | `{success, wins, matches, rank:{playlist,tierName,tierIconUrl,mmr}|null, cached, stale}` — bei Fehler zusätzlich `trackerStatus`/`trackerBodySnippet` oder `foundSegmentTypes` |
...
| `api/twitch-subs.js` | Echte Gesamt-Abonnentenzahl über Helix `GET /subscriptions` (für `oldschool-chatting.html`s Sub-Ziel-Balken) | `{success, total}` |

Alle drei cachen 30–60s In-Memory (modulweite Variable, gilt nur pro warmer Vercel-Function-Instanz).

**Rocket-League-Stats sind dauerhaft offline** (`api/stats.js` liefert `success:false`, alle RL-Stats-Widgets zeigen ihr D5/DANGSXR1000-Platzhalterbild) — **bewusst akzeptierte Einschränkung, kein Bug**. Tracker Network sperrte zunächst die private/undokumentierte `api.tracker.gg`-Adresse per Bot-Schutz (HTTP 403), und ihre offizielle Drittentwickler-API bietet schlicht **keine Rocket-League-Unterstützung an** (bestätigt über drei verschiedene Doku-Seiten in `tracker.gg/developers` — die "Spiel-APIs"-Liste zeigt konsequent nur Apex Legends und Die Division 2). Siehe Abschnitt 10 für die volle Diagnose-Historie.
...
0. **"90s Old School Edition"-Szenen: Live-Validierung läuft bereits** (`oldschool-starting/-chatting/-gameplay/-brb/-outro.html`). Drei Live-Test-Runden bisher (siehe `CLAUDE.md` für volle Details): (1) `oldschool-starting.html` — Social-Chips auf nur noch Twitch reduziert (alle Szenen), Countdown-Clipping per neuer `fitCountdownText()`-Auto-Fit-Funktion behoben, tanzender Bär ergänzt. (2) `oldschool-chatting.html`/`oldschool-brb.html` — leerer Facecam-Rahmen (war für eine im Projekt nie vorhandene echte Webcam gedacht) durch den Bären ersetzt, sichtbarer Chat-Verbindungsstatus ergänzt (1:1 `chat-vibes.html`-Muster). (3) Chat-Box in `oldschool-chatting.html` war trotz Fix (2) im echten Streamlabs-Output unsichtbar — Ursache: `.chat-box`s `left:1300vu`-Positionierung lag nahe der `--vu`-Clamp-Untergrenze-Bruchstelle (identischer Fehlerfall wie `overlay.html`-Hotfix-Historie Runde 2), auf `right:60vu` umgestellt, `oldschool-brb.html`s Chat-Box aus Konsistenzgründen ebenfalls auf robuste `left:50%`/`translateX(-50%)`-Zentrierung umgestellt. **Noch offen**: Bestätigung ob Runde-3-Fix live tatsächlich wirkt (Screenshot ausstehend), `oldschool-gameplay.html`/`oldschool-outro.html` noch gar nicht live getestet, F1/F4/F5s atmosphärischer CSS-Hintergrund vs. optionale eigene Medienquelle im selben Szenen-Slot, F3s Safe-Zone-Koordinaten gegen das echte Rocket-League-HUD (nur grob geschätzt), sowie ob `api/twitch-subs.js` nach Deploy tatsächlich `success:true` mit dem korrekten `total`-Wert liefert.
1. ~~`api/stats.js` / Rocket-League-Stats-Offline-Status~~ **Final geklärt — kein offener Punkt mehr.** Diagnose-Kette: Header-Fix gegen die Tracker.gg-Bot-Sperre wirkungslos → Umstieg auf die offizielle Tracker-Network-API (`TRACKER_API_KEY`) → erster URL-Versuch (`/v2/...`) lieferte `trackerStatus: 401` → korrigiert auf den offiziell dokumentierten Pfad `/api/v1/rocket-league/standard/profile/{platform}/{playerId}` → weiterhin `401 "Invalid authentication credentials"`. Nutzer prüfte daraufhin drei Doku-Seiten im Tracker.gg-Entwicklerportal (Erste Schritte, Datenaufnahme, Authentifizierung) — die "Spiel-APIs"-Liste zeigt konsistent nur Apex Legends und Die Division 2, **Rocket League wird von Tracker Network für Drittentwickler nicht angeboten**, keine Nutzer-seitige Freischaltung möglich. Per `AskUserQuestion` entschieden: Offline-Fallback-Panel bleibt **dauerhafter, akzeptierter Zustand** — keine weiteren Fix-Versuche an `api/stats.js` unternehmen, außer der Nutzer meldet zurück, dass eine individuelle Anfrage bei `business@tracker.network` doch noch Zugang ergeben hat.
