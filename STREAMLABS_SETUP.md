...
| Live Deck (`oldschool-live.html`/`-studio.html`) reagiert nicht, solange im Spiel/GeForce NOW aktiv ist — **auch mit „Live (Echtzeit)" im HUD** | Live bestätigt (11.08.2026): selbst eine aktive Pusher-Verbindung hilft nicht — vermutlich friert Android den gesamten Streamlabs-App-Prozess ein, nicht nur einzelne Seiten-Timer. Aktuell keine bekannte Code-Lösung dafür | Szene möglichst vor dem Spielwechsel schon setzen (einzige zuverlässige Methode); neue Recherche läuft (`.claude/stream-setup.md`, native Streamlabs-Lösung/Android-App-Freeze-Ausnahme) |
