...
11. **Modul 4**: Animierte Soundtrack-Badge in `goal.html` (Text aus einem `?track=`-URL-Parameter, gleiches bewährtes Prinzip wie `chat.html`s `?channel=`-Parameter — Aendern erfolgt ueber die Browser-Quellen-URL in Streamlabs Mobile, kein Redeploy noetig). Default-Text falls Parameter fehlt: "🎵 SOUNDTRACK: StreamBeats Hip-Hop (DMCA-Safe)".
12. **Modul 3**: `avatar.html` (audio-reaktiver PNGtuber/Masken-Avatar) — erst Mikro-Permission-Spike auf dem echten Tablet (Streamlabs-Mobile-WebView-Verhalten ungetestet), dann volles CSS/SVG-Maskendesign. Höchste technische Unsicherheit im ganzen Projekt, bewusst zuletzt eingeplant.
