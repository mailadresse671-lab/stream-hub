...
- **`avatar.html`** — **Mini-Spike (Modul 3, Schritt 1)**, noch kein fertiges Maskendesign. Testet ausschließlich, ob `getUserMedia()`-Mikrofon-Berechtigungen in der Streamlabs-Mobile-Android-WebView überhaupt durchgereicht werden. `?test=speak` simuliert einen Sprech-Pegel (Sinus + Rauschen) ohne echtes Mikro, gleiches Prinzip wie `goal.html`s `?test=`. Bei echtem Mikro: `getUserMedia({audio:true})` → `AnalyserNode` liest den Pegel, treibt eine simple Kreisform (Idle-Wippen via CSS, Sprech-Puls via JS-Transform auf getrenntem inneren Element, gleiches Architekturprinzip wie `#party-popup`/`#party-panel`). Der Analyser-Node wird bewusst **nicht** mit `audioCtx.destination` verbunden, um Mikro-Feedback/Echo im Stream zu vermeiden. Bei verweigerter/fehlender Berechtigung: dauerhafter Idle-Fallback (kein Absturz, kein Retry-Loop), der `err.name` der Ablehnung wird im sichtbaren Debug-Text angezeigt, damit sich auf dem Tablet ohne Inspector diagnostizieren lässt, woran es liegt.
...
11. **Modul 3, Schritt 2**: Wartet auf die Live-Bestätigung von `avatar.html` (siehe Punkt 1) — danach entweder volles CSS/SVG-Maskendesign (falls Mikro funktioniert) oder Umstieg auf eine rein visuelle Idle-Version ohne Mikro-Reaktivität.
12. **`overlay.html`** (All-In-One Master-Szene, 1920×1080): kombiniert Wanted-Level, Follower-Goal, Rang/MMR-HUD, Soundtrack-Badge, Chat-Feed, Avatar und alle Alert-/FX-Layer auf einer einzigen Browser-Quelle — geplant als Ergänzung, NICHT als Ersatz der bestehenden Einzelseiten (die bleiben als modulare Backup-Szenen bestehen). Komponenten-Layout steht (siehe Session-Notizen), Umsetzung startet erst nach Live-Bestätigung von Modul 3, damit der Avatar-Teil nicht doppelt gebaut werden muss.
13. Phase 3 der Roadmap (Chat-TTS, Twitch-Schedule einrichten — Channel-Points-Rewards/Chatbot-Commands sind durch Modul 1 teilweise bereits vorweggenommen).
14. Affiliate-Status abwarten (0/10 Abonnenten-Punkte) — Emote-/Sub-Badge-Redesign erst danach sinnvoll (Stufe 2/3 sind vorher gesperrt).
