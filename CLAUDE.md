...
- **`avatar.html`** — Audio-reaktiver Hood-Masken-Avatar (Modul 3). **Live-Test bestätigt**: `getUserMedia()` wird in der Streamlabs-Mobile-Android-WebView blockiert (`NotAllowedError`, kein Berechtigungsdialog erscheint überhaupt — im normalen Mobile-Chrome funktioniert es einwandfrei, das Problem sitzt eindeutig in der restriktiven WebView). Design-Konsequenz: eine dauerhafte, mikrofonunabhängige Animation ist der **primäre** Zustand, nicht mehr nur Fallback. Sturmhauben-Silhouette per `clip-path`-Polygon (gleiches Vokabular wie `goal-card`/`#party-panel` — zerklüftete Kante statt runder Ecken, kleine Riss-Akzente in Blutrot, Mittelnaht), zwei glühende gold Augen. Drei Verhaltens-Ebenen: (1) **Idle** — CSS-Keyframes für Atmen (Skalierung/Wiegen) + Augen-Glimmern, plus zufälliges Blinzeln alle 2,6–6s via JS-Timer. (2) **Chat-reaktiv** — eigene `tmi.js`-Verbindung (in try/catch gekapselt, gleiches Muster wie `goal.html`s Chat-Commands) löst bei jeder eingehenden Nachricht einen kurzen Reaktions-Puls aus (Augen weiten sich/leuchten heller) — direkte Umsetzung des Kernprinzips „Chat ersetzt die Stimme". (3) **Mikro-Bonus** — `getUserMedia`-Pfad bleibt kostenlos im Code (färbt die Augen blutrot statt gold, wenn aktiv), schlägt er fehl, läuft Idle/Chat unverändert weiter, kein sichtbarer Fehlertext mehr (das war nur für den Mini-Spike gedacht). `?test=speak` simuliert weiterhin einen Mikro-Pegel zu Testzwecken.
...
- **Modul 3 (Avatar)**: `avatar.html` — Live-Test bestätigte, dass Mikro-Zugriff in der Streamlabs-Mobile-WebView blockiert ist. Vollständiges Hood-Masken-Design umgesetzt: dauerhafte Idle-Animation (Atmen, Augen-Glimmern, zufälliges Blinzeln) + Chat-reaktiver Puls via `tmi.js` (Kernprinzip „Chat ersetzt die Stimme") als primärer Zustand, Mikro-Reaktivität bleibt als kostenloser Bonus-Pfad im Code. Per Playwright getestet (Reaktions-Mechanik, Cooldown, automatisches Blinzeln, Idle-Animation trotz blockiertem tmi.js/Mikro, `?test=speak`-Simulation).
...
11. **`overlay.html`** (All-In-One Master-Szene, 1920×1080): kombiniert Wanted-Level, Follower-Goal, Rang/MMR-HUD, Soundtrack-Badge, Chat-Feed, Avatar und alle Alert-/FX-Layer auf einer einzigen Browser-Quelle — geplant als Ergänzung, NICHT als Ersatz der bestehenden Einzelseiten (die bleiben als modulare Backup-Szenen bestehen). In Arbeit.
